...
- الملف: **`TRANSLATIONS_MASTER.xlsx`** في جذر المستودع — **الإصدار 3 — 520 مسمى** (بعد حذف ميزات الاستوديو التسع، وباستثناء كل نصوص "المساعد الذكي" بالكامل):
  - ورقة "دليل الاستخدام": شرح الأعمدة + فهرس كل الأقسام الرئيسية مع عدد النصوص بكل قسم
  - ورقة "2- قاموس الواجهة": 215 مفتاح `I18N` مُجمّعة حسب فئة الاستخدام (24 فئة فرعية: الفلاتر، صفحة الأصل، إدارة الوسائط...)
  - ورقة "3- النصوص المضمّنة": 305 نص من `L('','')`، `L(\`\`,\`\`)` الديناميكي، `ar:/en:`، `dar:/den:`، و`LANG==='ar'?:` — مُجمّعة حسب **القسم الرئيسي** (الشاشة/الميزة) ثم **الموقع الفرعي** (الدالة البرمجية المحيطة)، مرتّبة حسب رقم السطر الفعلي في `index.html`، مع صفوف عناوين ملوّنة لكل قسم وتجميع Outline قابل للطي
- **آلية العكس عند إعادة رفع الملف المعدل**:
  - ورقة القاموس: استبدال قيمة المفتاح في كتلة `I18N` (`key:"قديم"` → `key:"جديد"`)
  - ورقة النصوص المضمّنة: استبدال نصّي دقيق للزوج القديم بالجديد في كامل `index.html` (أعمدة "الحالي" هي مفاتيح المطابقة — ممنوع تعديلها)
  - النصوص "الديناميكية" (تحتوي `${...}`) يجب الإبقاء على جزء `${...}` كما هو عند الترجمة
  - بعد العكس: فحص syntax لكل السكربتات + فحص Playwright قبل الدفع
- سكربت إعادة التوليد محفوظ محلياً في هذه الجلسة (`extract.py` + `build_xlsx.py` + `sections.py` + `i18n_categories.py`) — إن احتجت تحديث الملف مجدداً بعد تعديلات مستقبلية في الكود، أعد بناءه بنفس الطريقة (خرائط الأقسام بحاجة تحديث يدوي إذا تغيّرت بنية السكربتات).
...
TRANSLATIONS_MASTER.xlsx       ← ملف المسميات الشامل — الإصدار 3 (520، بدون المساعد الذكي) ⭐
...
- [x] ~~حذف الميزات التسع الجديدة من استوديو التراث~~ (تم — commit `50578d8`)
- [x] ~~إعادة بناء إكسل احترافي (الإصدار 3) بدون نصوص المساعد الذكي، مُصنّف حسب القسم/الموقع الفرعي~~ (تم)
- [ ] استقبال `TRANSLATIONS_MASTER.xlsx` المعدل (الإصدار 3 — 520 مسمى) وعكسه على الموقع (آلية بند 5)
